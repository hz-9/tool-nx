...
## 更新摘要
**所做更改**
- 新增 CLI 日志系统章节，介绍彩色输出和调试能力
- 更新文件处理统计信息部分，详细说明统计指标和可视化反馈
- 增强故障排查指南，包含统计信息相关的诊断方法
- 更新架构图表，反映新的统计信息收集和展示流程
...
**更新** 新增 CLI 日志系统和改进的文件处理统计信息，为高级用户提供更好的可视化反馈和调试能力。
...
B --> B6["src/bin<br/>CLI 入口"]
B6 --> B7["docs-build.ts<br/>命令行入口"]
B7 --> B8["ColorUtil<br/>彩色输出工具"]
B7 --> B9["统计信息展示"]
...
- [packages/docs-build/src/bin/docs-build.ts:1-50](file://packages/docs-build/src/bin/docs-build.ts#L1-L50)
- [packages/docs-build/src/util/color.util.ts:1-71](file://packages/docs-build/src/util/color.util.ts#L1-L71)
...
- **CLI 日志系统**：新增彩色输出和统计信息展示功能，提供更好的调试体验。

**更新** 新增 CLI 日志系统组件，包含 ColorUtil 工具类和统计信息展示功能。
...
## 架构总览
下图展示从命令行到模板渲染与输出的端到端流程，涵盖配置加载、文件扫描与复制、导航与侧边栏多语言转换、模板渲染与最终产物定位，以及新增的统计信息收集和展示流程。
...
participant Stats as "统计信息收集"
participant FS as "文件系统"
participant TPL as "模板引擎(Lodash)"
participant LOG as "CLI 日志系统"
...
- [packages/docs-build/src/bin/docs-build.ts:38-42](file://packages/docs-build/src/bin/docs-build.ts#L38-L42)
...
### CLI 日志系统与彩色输出
- **ColorUtil 工具类**：提供 ANSI 颜色码支持，包括粗体、各种颜色（红、绿、黄、蓝、洋红、青）和亮色变体。
- **NO_COLOR 环境变量支持**：检测 NO_COLOR 环境变量，当设置时禁用彩色输出，确保兼容性。
- **文本清理功能**：提供清除 ANSI 颜色码的功能，用于日志文件或非终端输出。
- **彩色输出应用场景**：在 CLI 中使用不同颜色区分统计信息类型，提高可读性和调试效率。

```mermaid
flowchart TD
Start(["CLI 日志系统初始化"]) --> CheckEnv["检查 NO_COLOR 环境变量"]
CheckEnv --> HasColor{"是否允许彩色输出?"}
HasColor --> |是| EnableColors["启用 ANSI 颜色码"]
HasColor --> |否| DisableColors["禁用彩色输出"]
EnableColors --> Bold["粗体文本支持"]
EnableColors --> Colors["彩色文本支持<br/>红/绿/黄/蓝/洋红/青"]
DisableColors --> Plain["纯文本输出"]
Bold --> Output["输出彩色日志"]
Colors --> Output
Plain --> Output
Output --> Clear["文本清理功能"]
...
**图示来源**
- [packages/docs-build/src/util/color.util.ts:5-71](file://packages/docs-build/src/util/color.util.ts#L5-L71)

**章节来源**
- [packages/docs-build/src/util/color.util.ts:1-71](file://packages/docs-build/src/util/color.util.ts#L1-L71)

### 改进的文件处理统计信息
- **统计信息结构**：新增 DocsBuildStats 接口，包含 totalScanned、totalCopied、totalDirectMapped 等关键指标。
- **统计信息收集**：在文件复制过程中实时收集统计数据，包括扫描总数、复制数量和直接映射数量。
- **彩色统计展示**：在 CLI 中使用 ColorUtil 工具类展示统计信息，不同类型的统计使用不同颜色标识。
- **调试能力增强**：通过详细的统计信息帮助用户了解文件处理过程，快速定位问题。
...
**图示来源**
- [packages/docs-build/src/core/docs-build.ts:126-140](file://packages/docs-build/src/core/docs-build.ts#L126-L140)
- [packages/docs-build/src/bin/docs-build.ts:38-42](file://packages/docs-build/src/bin/docs-build.ts#L38-L42)

**章节来源**
- [packages/docs-build/src/core/docs-build.ts:126-140](file://packages/docs-build/src/core/docs-build.ts#L126-L140)
- [packages/docs-build/src/bin/docs-build.ts:38-42](file://packages/docs-build/src/bin/docs-build.ts#L38-L42)
- [packages/docs-build/src/interface/options.ts:109-115](file://packages/docs-build/src/interface/options.ts#L109-L115)
...
- 内部依赖关系
  ...
  - **CLI 日志系统**：ColorUtil 依赖 NO_COLOR 环境变量检测，提供彩色输出功能。
...
- [packages/docs-build/src/core/docs-options.ts:1-68](file://packages/docs-build/src/core/docs-options.ts#L1-L68)
- [packages/docs-build/src/core/docs-build.ts:1-361](file://packages/docs-build/src/core/docs-build.ts#L1-L361)
- [packages/docs-build/src/util/color.util.ts:5-7](file://packages/docs-build/src/util/color.util.ts#L5-L7)
...
- **统计信息性能影响**
  - 统计信息收集采用轻量级计数器，对整体性能影响极小。
  - 彩色输出仅在 CLI 中启用，不影响非交互式输出。

**更新** 新增统计信息性能影响说明，强调其对整体性能的最小影响。
...
- **统计信息显示异常**
  - 现象：CLI 中不显示统计信息或显示异常。
  - 处理：检查 NO_COLOR 环境变量设置，确认 ColorUtil 工具类正常工作；验证 DocsBuildStats 接口定义。
- **彩色输出问题**
  - 现象：CLI 输出不显示彩色效果。
  - 处理：检查终端支持 ANSI 颜色码，确认 ColorUtil.isColorAllowed 返回正确值；检查 NO_COLOR 环境变量。

**更新** 新增统计信息显示异常和彩色输出问题的故障排查指南。
...
## 结论
该工具通过"配置驱动 + 模板渲染"的方式，提供了强大的多语言站点生成能力。结合 Nx 缓存与 Actions 流水线，可在大型项目中实现高效、稳定的文档发布。**新增的 CLI 日志系统和改进的文件处理统计信息**为高级用户提供了更好的可视化反馈和调试能力，通过彩色输出和详细的统计指标帮助用户更好地理解和监控文档构建过程。对于高级用户，建议围绕模板扩展、脚本编排与配置治理来实现更复杂的定制需求。

**更新** 强调新增 CLI 日志系统和统计信息功能对高级用户的实际价值。
